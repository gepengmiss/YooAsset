...
namespace SVC {
    ...
    public static class FileHelper {
        public static void WriteToFile(StringBuilder sb, string filePath, bool openFile = true) {  
            // string filePath = "Temp/ParsedCombinations.txt";     
            File.WriteAllText(filePath, sb.ToString()); 
            filePath = Application.dataPath.Replace("Assets", filePath).Replace("/", "\\"); 
            UnityEngine.Debug.Log($"shader分析结果保存位置：  {filePath}"); 
            if (openFile)
            {
