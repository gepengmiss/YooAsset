...
namespace SVC 
{
    ...
    public class ShaderParsedCombinations {
        ...
        public String Dump(bool log = false) {
            String result;
    ...
    public static class ShaderUtils {
        ...
        public static ShaderParsedCombinations ParseShaderCombinations( Shader shader, bool usedBySceneOnly, bool withLog = true ) {
            ...
            //     Debug.LogError(shader.name + " deps[ i ]: " + deps[ i ]);
            ...
            // Debug.LogError(shader.name + " lastWriteTime: " + lastWriteTime);
                    ...
                    for ( int i = 0; i < lines.Length; ++i ) {
                        ...
                        if ( match.Success ) {
                            ...
                            if ( curSnippet.variants != null ) {
                                ...
                                curSnippet.variants.Add( new String[] {ShaderParsedCombinations.TAG_NO_KEYWORDS_DEFINED} );
                            }
            ...
            // s_ShaderParsedCombinationsCache.Add( shader, new ShaderParsedCombinationsItem { data = result, lastWriteTime = lastWriteTime } );
            return result;
        }
    ...
    public static class ShaderVariantCollectionHelper {
        ...
        public static RawData GetShaderVariantEntries( Shader shader ) {
            ...
            var toolSVC = new ShaderVariantCollection();
            object[] args = new object[] { shader, 256, filterKeywords, toolSVC, _passtypes, _keywords, _remainingKeywords };

            MethodInfo  GetShaderVariantEntriesFiltered = typeof(ShaderUtil).GetMethod("GetShaderVariantEntriesFiltered", BindingFlags.NonPublic | BindingFlags.Static);
            // Debug.Log("GetShaderVariantEntriesFiltered: " + GetShaderVariantEntriesFiltered);
            GetShaderVariantEntriesFiltered.Invoke(null, args);
 
            var types = args[4] as int[]; 
            //key word 
            var keywords = args[5] as string[]; 
            //Remaning key word
            var remainingKeywords = args[6] as string[];
    ...
    public class SVCDataCache {
        public ShaderVariantCollection svc = new ShaderVariantCollection();
        public void Init()
        { 
            svc = new ShaderVariantCollection();
            Debug.LogError("--------- Init" + svc);
        }
        public void Add(ShaderVariant _sv)
        { 
            Debug.LogError("--------- Add" + svc);
            svc.Add(_sv);
        }
        public void Clear()
        {
            Debug.LogError("--------- Clear" + svc);

            if(svc != null) {
                svc.Clear();
            } 
            svc = new ShaderVariantCollection();
             Debug.LogError("--------- 2 Clear" + svc);
        }
        public void Save()
        {
