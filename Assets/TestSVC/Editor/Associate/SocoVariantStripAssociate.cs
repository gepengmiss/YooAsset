...
public class SocoVariantStripAssociate : Soco.ShaderVariantsCollection.IExecutable
{
    public override void Execute(ShaderVariantCollectionMapper mapper)
    {
        ...
        var stripConfigs =
            // com.cyou.soco
            Soco.ShaderVariantsStripper.ShaderVariantsStripperCode.LoadConfigs();
        ...
        foreach (var shader in mapper.shaders)
        {
            ...
            foreach (var variant in variants)
            {
                ...
                Debug.LogError($"1 变体剔除 Shader:{shader.name}  正在处理{shader.name} {variantIndex}/{variants.Length}变体");
                ...
                if (
                    //com.cyou.soco
                    Soco.ShaderVariantsStripper.ShaderVariantsStripperCode.IsVariantStrip(shader, snipperData, data, stripConfigs))
                {
